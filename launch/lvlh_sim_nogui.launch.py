...
def generate_launch_description():
    ...
    # 1. Gazebo Simulation - HEADLESS MODE (GUI 없음)
    start_world = ExecuteProcess(
        cmd=[
            'gz', 'sim', 
            world_file, 
            '-r',      # Run on start
            '-s',      # Server only mode (no GUI)
            '--headless-rendering'  # 헤드리스 렌더링 활성화
        ],
        output='screen',
        additional_env=env
    )

    # 2. Camera Bridge Node - 그대로 유지 (이미지 토픽은 계속 발행됨)
    ...
    # 5. ROS Bridge WebSocket Server - 웹 인터페이스용
    ...
    # 6. Web Video Server Node - 웹에서 이미지 스트리밍
