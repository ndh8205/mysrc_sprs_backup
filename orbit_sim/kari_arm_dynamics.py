#!/usr/bin/env python3
"""
kari_arm_dynamics.py
KARI ARM 동역학 라이브러리 (Free-floating Space Manipulator)
...
class KariArmParams:
    ...
    def __init__(self):
        ...
        self.r_mount = np.array([0.0, 0.0, 0.5])  # 장착점 (위성 body frame)
...
if __name__ == "__main__":
    ...
    # FK
    fk = forward_kinematics(params, p_base, q_base, theta)
    print(f"EE Position: {fk.p_ee}")
    print(f"EE Position (expected ~[0, -0.384, 2.447])")
    
    # Mass Matrix
    H = compute_mass_matrix(fk, params)
    print(f"\nMass Matrix shape: {H.shape}")
    print(f"Total mass (H[0,0]): {H[0,0]:.2f} kg")
    
    # Jacobian
    Jb, Jm = compute_jacobian(fk, params)
    print(f"\nJb shape: {Jb.shape}, Jm shape: {Jm.shape}")
    
    # Generalized Jacobian
    Jg = compute_generalized_jacobian(fk, H, params)
    print(f"Jg shape: {Jg.shape}")
